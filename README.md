...
Nesse pimeiro momento, só estou compartilhando a parte do lovelace. Estarei realizando demais ajustes no projeto e aos poucos vou compartilhando os demais arquivos. Mas o que compartilho é o suficiente para a parte do design.
...
- [Day Countdown](https://github.com/bundito/day-countdown): Instalação pelo HACS. Como você pode perceber na página do desenvolvedor, desde abril o mesmo não está mais realizando ajustes nem dando suporte. Portando, eu pretendo mudar em breve esse cartão. Ele serve paenas para realizar uma contagem em dias para uma determinada data. Se não vai utilizar esse recurso, não é necessário.
- [Ext Weblink](https://github.com/custom-cards/ext-weblink): Instalação pelo HACS. Esse card é super simples, apenas para facilitar a criação de textos e links com imagens. Não é necesário.
...
- [Card-Tools](https://github.com/thomasloven/lovelace-card-tools): Instação manual através do HACS. É um requisito para utilizar outros cartões. É obrigado a ter esse cartão instalado.
- [Popup-Card](https://github.com/thomasloven/lovelace-card-tools): Instação manual através do HACS. Esse cartão é o que faz a mágica para abrir as janelas/modais no meu projeto. Sem isso, os botões da páginma principal não irão funcionar.
- [Thermostat-Card](https://github.com/thomasloven/lovelace-card-tools): Instação manual. Esse cartão é uma exceção para realizar a instalação. Não é possível instalar manualmente pelo HACS, nesse caso será preciso voltar a moda "antiga" e baixar os arquivos para instalar manualmente. Esse cartão serve para mostrar a temperatura dos ar condicionados. Não é necessário, você pode susbsituir por outros modelos de climatização.
- [Sun-Card](https://github.com/mishaaq/sun-card): Instação manual através do HACS. Esse cartão é onde mostra as informações do sol (por do sol e nascer do sol). Se você não vai utilizar, não é necessário.
...
- @jimz011 - Por compartilhar todo seu projeto e além disso responder todas as minhas dúvidas. Meu projeto é "ctrl+c" e "ctrl+v" do projeto dele.
- @PatteTech - Foi graças a seus vídeos, grupos que me fez entrar no mundo da automação, ajuda muita pessoas como eu que querem começar e tem diversas dúvidas.
- @marciogranzotto - Por compartilhar suas automações incríveis, como pegar as taxas de energia elétrioca da empresa CELESC.
- @htcheroportugal - Co-Fundador CPHA, uma comunidade portuguesa muito forte com projetos utilizando o HA, e par anós brasileiro é muito importante, pois entendemos perfeitamente seus tutoriais, vídeos, etc. E além disso são muito atenciosos e organizados. Sempre que precisei me ajudaram de várias formas.
